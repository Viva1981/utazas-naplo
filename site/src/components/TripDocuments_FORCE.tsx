    ...
    <section className="max-w-5xl mx-auto mt-10 p-6 rounded-xl shadow-md" style={{background:"#fff8ee"}}>
      ...
      {isOwner && (
        <form onSubmit={onUploadDocs} className="flex flex-wrap gap-4 items-center border p-4 rounded-lg bg-gray-50 mb-6">
          ...
          <button type="submit" className="px-4 py-2 bg-amber-600 text-white text-sm font-medium rounded-md hover:bg-amber-700 transition">
            Feltöltés
          </button>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {visibleDocs.map((m) => {
            ...
            return (
              <article
                key={m.id}
                className={`group border rounded-xl shadow-sm hover:shadow-lg transition overflow-hidden cursor-pointer ${m.media_visibility === "public" ? "bg-white" : "bg-gray-50"}`}
                onClick={() => onOpen(m)}
              >
                <div className="relative aspect-[4/3] bg-gray-100 overflow-hidden">
                  {isImage ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center bg-white text-red-600 text-xs">PDF előnézet</div>
                  ) : thumb ? (
                    <img src={thumb} alt={m.title || "Dokumentum"} className="w-full h-full object-contain bg-white" />
                  ) : (
                    <div className="flex items-center justify-center h-full text-4xl">📄</div>
                  )}

                  {m.media_visibility !== "public" && (
                    <span className="absolute top-2 right-2 bg-black/60 text-xs text-white px-2 py-1 rounded-full">Privát</span>
                  )}
                </div>
                ...
                <div className="p-4">
                  ...
                  {isOwner && (
                    <button
                      ...
                      className="mt-2 text-xs text-red-600 border border-red-300 rounded-md px-3 py-1 hover:bg-red-50 transition"
                    >
                      Törlés
                    </button>
                  )}
      ...
      {open && active && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4" onClick={onClose}>
          <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-5xl h-[85vh] overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <button onClick={onClose} className="absolute top-3 right-3 z-10 bg-black/60 text-white rounded-full px-3 py-1 text-sm">Bezárás ✕</button>
